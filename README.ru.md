...
**v0.7 — alpha.** Основная идея подтверждена от начала до конца полным набором
тестов (163 офлайн-теста, покрытие ~95%) и зелёным CI (ruff + **mypy** + тесты).
v0.7 добавляет экспортёр OpenTelemetry и обновлённый UI оболочки; v0.6 — кассеты
записи/воспроизведения; v0.5 — провайдер OpenAI, интерактивное подтверждение и
мост к MCP; v0.4 — локальные модели через Ollama; v0.3 — десктоп-оболочку; v0.2
— стриминг, параллельные инструменты и подтверждение. API ещё может меняться до
1.0. Обратная связь и issues приветствуются.
