...
| **Граница провайдера** | один протокол `LLM` (~40 строк); адаптеры Claude, локальные (Ollama) и офлайн |
...
**v0.4 — alpha.** Основная идея подтверждена от начала до конца полным набором
тестов (129 офлайн-тестов) и зелёным CI. v0.4 добавляет локальные модели через
Ollama; v0.3 — скачиваемую десктоп-оболочку; v0.2 — стриминг, параллельные
инструменты и human-in-the-loop подтверждение. API ещё может меняться до 1.0.
