...
**v0.5 — alpha.** Основная идея подтверждена от начала до конца полным набором
тестов (151 офлайн-тест, покрытие ~95%) и зелёным CI (ruff + **mypy** + тесты).
v0.5 добавляет провайдер OpenAI, интерактивное подтверждение инструментов в
десктоп-оболочке и мост к MCP; v0.4 — локальные модели через Ollama; v0.3 —
десктоп-оболочку; v0.2 — стриминг, параллельные инструменты и подтверждение. API
ещё может меняться до 1.0. Обратная связь и issues приветствуются.
